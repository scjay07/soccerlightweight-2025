...
int margem = 140;
float kp = 0.22;  // Ajuste fino do ganho proporcional
...
void alinhar() {
  //
  if (angulocorrecao > 0 && angulocorrecao <= 2) {
  }
  ...
  wfd = angulocorrecao;
  wtd = angulocorrecao;
  wfe_v = (byte)map(abs(wfe), 0, 180 * kp + margem , 17, 200);
  wfd_v = (byte)map(abs(wfd), 0, 180 * kp + margem , 17, 200);
  wte_v = (byte)map(abs(wte), 0, 180 * kp + margem, 17, 200);
  wtd_v = (byte)map(abs(wtd), 0, 180 * kp + margem, 17, 200);
  sentidomotor(wfe, 0);
  sentidomotor(wfd, 1);
  sentidomotor(wte, 2);
  sentidomotor(wtd, 3);
...
void loop()
{
  if (Serial2.available())
  {
    ...
    { //se o robô estiver desalinhado em até 50 graus, então alinha);
      tempoinicial = millis();
      while ((abs(angulolido) > 20 && abs(angulolido) < 350) && (millis() - tempoinicial) <= 75)
      {
        mudar_cor(0, 0, 255);
